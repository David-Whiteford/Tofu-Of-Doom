...
void GameOver::update(sf::Time t_deltaTime)
{
	...
	std::cout << "TIMER: " << screenTimer.asSeconds() << std::endl;
}
/// <summary>
/// render to t_window
/// </summary>
...
void GameOver::setUpContent()
{
	...
	m_titlemessageSecond.setPosition(1170.0f, 530.0f);
...
void GameOver::screenTransitionOn(sf::Time t_deltaTime)
{
	...
	if (transitionTimer >= sf::seconds(0.0) && transitionTimer <= sf::seconds(5.0f))
	{
		...
		m_backgroundSprite.setColor(sf::Color(255, 255, 255, m_backgroundSprite.getColor().a - 0.1f));
		m_titlemessageSecond.setColor(sf::Color(255, 255, 255, m_backgroundSprite.getColor().a - 0.1f));
		m_titleMessage.setColor(sf::Color(255, 255, 255, m_backgroundSprite.getColor().a - 0.1f));
	}
